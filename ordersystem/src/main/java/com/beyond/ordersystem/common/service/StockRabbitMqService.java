...
public class StockRabbitMqService {
    ...
    public void publish(Long productId, int productCount) {
        StockRabbitMqDto stockRabbitMqDto = StockRabbitMqDto.builder().productId(productId).productCount(productCount).build();
        rabbitTemplate.convertAndSend("stockDecreaseQueue", stockRabbitMqDto);
    }

    // rabbitmq에 발행된 메시지를 수신
    // listener는 단일 스레드로 메시지를 처리하므로, 동시성이슈 발생 X
    ...
    public void subscribe(Message message) throws JsonProcessingException {
        ...
        Product product = productRepository.findById(dto.getProductId()).orElseThrow(() -> new EntityNotFoundException("no no"));
